...
| Do you provide a way for users to request that their data is deleted? | **Data not collected** (n/a) — ⚠ becomes **Yes** when #258's opt-in report send ships; change it in the SAME PR as the client send path, per `store-submission.md` gate 1 | Nothing exists server-side **today**; on-device data is deleted via in-app controls / Clear data / uninstall — stated in the privacy policy. Once the send path ships, the Worker retains submitted reports for 180 days and the salted rate-limit identifier for one hour (`report-worker/src/index.ts`), so "nothing exists server-side" stops being true and the answer becomes deletion-on-request via the contact email. |
